...
async def verify_email(*,session:async_session,token:str,email:str):
    try:
        ...
        if r_token == token:
            ...
            return RedirectResponse(url="http://localhost:8000/v1.0.0/user/verification-successfull")
        raise Exception("Token expired")
    except Exception :
        return RedirectResponse(url="http://localhost:8000/v1.0.0/user/verification-failed")
            

@route.get("/verification-successfull",response_class=HTMLResponse)
async def verification_success():
    path = Path(r"D:\Projects\Fickel\Monolithic\backend\app\template\verificationsuccess.html")
...
async def verification_failed():
    path = Path(r"D:\Projects\Fickel\Monolithic\backend\app\template\verficationfailed.html")
    with open(path,"r") as file:
        html_file = file.read()
    return html_file

@route.patch("/change_password",status_code=204)
async def Password_change(session:async_session,password:changePassword,fickel_token: Annotated[str | None, Cookie()] = None):
